...
def configure_matplotlib_fonts():
    ...
    # 云端字体（Noto CJK）
    cloud_fonts = ["Noto Sans CJK SC", "Noto Sans CJK TC", "Noto Serif CJK SC"]

    # 合并所有候选字体
    all_candidates = cloud_fonts + font_candidates

    # 获取系统可用字体
    available = {font.name for font in font_manager.fontManager.ttflist}

    # 选择第一个可用的字体
    selected = [name for name in all_candidates if name in available]

    if selected:
        plt.rcParams["font.sans-serif"] = selected + list(plt.rcParams.get("font.sans-serif", []))
    else:
        # 如果没有找到指定字体，尝试使用任何CJK字体
        cjk_fonts = [font.name for font in font_manager.fontManager.ttflist
                     if 'CJK' in font.name or 'Noto' in font.name]
        if cjk_fonts:
            plt.rcParams["font.sans-serif"] = [cjk_fonts[0]] + list(plt.rcParams.get("font.sans-serif", []))
